...
void updateDisplay() {
  ...
  u8g2.drawStr(3, 13, "Selected Device:");
  int x = 0;
  int y = 35;
  ...
  u8g2.drawStr(3, 58, "SPAMMER:");
  u8g2.setCursor(70, 58);
  u8g2.print(isAdvertising ? "Activated" : "Deactivated");
...
void toggleAdvertising() {
  ...
  while (isAdvertising) {
    ...
    if (digitalRead(advControlPin) == LOW) {
      ...
      isAdvertising = !isAdvertising;
      setNeoPixelColour("off");  // Turn off NeoPixel
      updateDisplay(); // Update display to show "Disabled"
      break;  // Exit the loop
    }
